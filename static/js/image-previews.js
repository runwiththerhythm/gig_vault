...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderSummary(files) {
    ...
    if (summary) {
      const firstThree = Array.prototype.slice.call(files, 0, 3).map(function (f) { return f.name; }).join(", ");
  ...
  function mergeIntoInput(newFiles) {
    ...
    try {
      ...
      // Older browsers: bail gracefully (no removal/merge), but keep previews working
      console.warn("DataTransfer not supported in this browser; skipping merge.");
      return;
    }
  ...
  function handleFiles(selected) {
    const files = Array.prototype.slice.call(selected);
    const ok = [], errs = [];
